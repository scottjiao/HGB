...
for dataset,worker_num in dataset_to_evaluate:
    ...
    for gpu in gpus:
        for _ in range(worker_num):
            command=f"python main_sparse.py --GNN GTN --dataset {dataset} --gpu {gpu} --trial_num {trial_num} --study_name {study_name} --study_storage {study_storage}"
            p=Run(command)
            p.daemon=True
            p.start()
